...
public class LoadMyBookmarkPostTest extends BaseTest {
    ...
    private final List<FieldDescriptor> responseFieldLoadMyBookmarkPosts =
            List.of(
    ...
    private ResultActions getResultActions(MockHttpSession session, int page, int size)
        ...
        return mockMvc.perform(
                RestDocumentationRequestBuilders.get("/api/users/me/bookmarks")
    ...
    private RestDocumentationResultHandler getDocument(Integer identifier) {
        return document(
                "loadMyBookmarkPosts/" + identifier,
                preprocessRequest(prettyPrint()),
                preprocessResponse(prettyPrint(), getModifiedHeader()),
                responseFields(responseFieldLoadMyBookmarkPosts),
                queryParameters(queryParametersForPage, queryParametersForSize),
                resource(
                        ResourceSnippetParameters.builder()
                                .tag("회원")
                                .summary("북마크한 유저픽 게시글 목록 조회 api")
                                .description("북마크한 유저픽 게시글 목록을 조회하는 api 입니다.")
                                .responseFields(responseFieldLoadMyBookmarkPosts)
                                .queryParameters(queryParametersForPage, queryParametersForSize)
                                .build()));
    }
    ...
    void set_up() throws Exception {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            CreateBookmarkCommand createBookmarkCommand =
                    CreateBookmarkCommand.of(user.getId(), post.getId());
            createBookmarkUseCase.execute(createBookmarkCommand);
        }
    }

    @Test
    @Transactional
    void 북마크한_유저픽_게시글_목록_조회_성공() throws Exception {
        // given
    ...
    @Test
    @Transactional
    void 북마크한_유저픽_게시글_목록_조회_실패1() throws Exception {
        // when
    ...
    @Test
    @Transactional
    void 북마크한_유저픽_게시글_목록_조회_실패2() throws Exception {
        // given
        MockHttpSession session = login("test@gmail.com");
    ...
    @Test
    @Transactional
    void 북마크한_유저픽_게시글_목록_조회_실패3() throws Exception {
        // given
        MockHttpSession session = login("test@gmail.com");
