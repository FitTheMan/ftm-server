...
public class LoadPostProductHashTagsTest extends BaseTest {
    ...
    private final List<FieldDescriptor> responseFieldLoadPostProductHashTags =
            List.of(
                    ...
                    fieldWithPath("data.results[]").type(ARRAY).description("상품 해시태그 목록 결과"),
                    fieldWithPath("data.results[].category")
                            .type(OBJECT)
                            .description("상품 해시태그 카테고리"),
                    fieldWithPath("data.results[].category.name")
                            .type(STRING)
                            .description("상품 해시태그 카테고리 이름"),
                    fieldWithPath("data.results[].category.label")
                            .type(STRING)
                            .description("상품 해시태그 카테고리 라벨"),
                    ...
                    fieldWithPath("data.results[].hashtags[].name")
                            .type(STRING)
                            .description("상품 해시태그 이름. 상품 생성/수정 시 전달"),
                    fieldWithPath("data.results[].hashtags[].tag")
                            .type(STRING)
                            .description("상품 해시태그 태그 이름"));

    private ResultActions getResultActions() throws Exception {
        return mockMvc.perform(
                RestDocumentationRequestBuilders.get("/api/posts/products/hashtags"));
    }

    private RestDocumentationResultHandler getDocument(Integer identifier) {
        return document(
                "loadPostProductHashTags/" + identifier,
                preprocessResponse(prettyPrint(), getModifiedHeader()),
                responseFields(responseFieldLoadPostProductHashTags),
                resource(
                        ResourceSnippetParameters.builder()
                                .tag("유저픽 게시글")
                                .summary("상품 해시태그 목록 조회")
                                .description(
                                        "상품 해시태그 목록을 조회하는 api 입니다. 상품 생성/수정 시 해시태그의 영문 이름 정보를 전달해주세요.")
                                .responseFields(responseFieldLoadPostProductHashTags)
                                .build()));
    }
