...
public class LoadUserPickAllPopularPostsTest extends BaseTest {
    ...
    private final List<ParameterDescriptor> queryParameters =
            List.of(
                    ...
                    parameterWithName("lastPostId")
                            .optional()
                            .description("이전 로딩의 lastId 입력")
                            .attributes(
                                    new Attributes.Attribute(
                                            "constraint",
                                            "이전 스크롤 로딩 응답값(lastPostId) 필드 그대로 사용. 첫번째 스크롤인 경우 전달 X")));
    ...
    private final List<FieldDescriptor> responseFields =
            List.of(
                    ...
                    fieldWithPath("data.lastScore")
                            .type(STRING)
                            .optional()
                            .description("다음 스크롤 로딩 요청 시, 요청 파라미터 lastScore의 값이 됨")
                            .attributes(
                                    new Attributes.Attribute(
                                            "nullable", "hasNext가 false인 경우 null")),
                    fieldWithPath("data.lastPostId")
                            .type(NUMBER)
                            .optional()
                            .description("다음 스크롤 로딩 요청 시, 요청 파라미터 lastPostId의 값이 됨")
                            .attributes(
                                    new Attributes.Attribute(
                                            "nullable", "hasNext가 false인 경우 null")),
    ...
    private ResultActions getResultActions() throws Exception {
        return mockMvc.perform(
                RestDocumentationRequestBuilders.get("/api/posts/userpick/all/popular")
                        .queryParam("limit", "5"));
    }

    private RestDocumentationResultHandler getDocument(Integer identifier) {
        return document(
                "loadUserPickAllPopular/" + identifier,
                ...
                resource(
                        ResourceSnippetParameters.builder()
                                .tag("유저픽 게시글")
                                .summary("\"그루밍 이야기\" 인기순 조회 api")
                                .description("그루밍 라운지 내 \"그루밍 이야기\" 인기순 목록 조회 api 입니다.")
                                .responseFields(responseFields)
                                .build()));
    }
