...
public class LoadProductsByHashTagTest extends BaseTest {
    ...
    private final List<ParameterDescriptor> queryParameters =
            List.of(
                    ...
                    parameterWithName("lastScore")
                            .description("이전 로딩의 lastScore 입력")
                            .optional()
                            .attributes(
                                    new Attributes.Attribute(
                                            "constraint",
                                            "이전 스크롤 로딩 응답값(lastScore) 필드 그대로 사용. 첫번째 스크롤인 경우 전달 X")));
    ...
    private final List<FieldDescriptor> responseFields =
            List.of(
                    ...
                    fieldWithPath("data").type(OBJECT).optional().description("예외 발생한 경우 null"),
                    fieldWithPath("data.data")
                            .type(ARRAY)
                            .description("응답 데이터. 데이터 없는 경우 빈 배열 반환")
                            .optional(),
                    fieldWithPath("data.data[].productId").type(NUMBER).description("상품 id"),
                    fieldWithPath("data.data[].productName").type(STRING).description("상품 이름"),
                    fieldWithPath("data.data[].productImage")
                            .type(STRING)
                            .description("상품 이미지 url"),
                    fieldWithPath("data.data[].likeYn")
                            .type(BOOLEAN)
                            .description("사용자 추천 버튼 누름 여부"),
                    fieldWithPath("data.data[].brand").type(STRING).description("브랜드"),
                    fieldWithPath("data.data[].recommendedCount").type(NUMBER).description("추천수"),
                    fieldWithPath("data.data[].postId").type(NUMBER).description("상품이 속한 게시글 id"),
                    fieldWithPath("data.hasNext").type(BOOLEAN).description("다음 스크롤에 데이터 존재 여부"),
                    fieldWithPath("data.lastScore").type(NUMBER).description("현재 스크롤의 마지막 점수"));
