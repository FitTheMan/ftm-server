...
public class CreatePostLikeTest extends BaseTest {
    ...
    private ResultActions getResultActions(Long postId, MockHttpSession session) throws Exception {
        return mockMvc.perform(
                RestDocumentationRequestBuilders.post("/api/posts/{postId}/like", postId)
                        .session(session));
    }

    private RestDocumentationResultHandler getDocument(Integer identifier) {
        return document(
                "createPostLike/" + identifier,
                ...
                resource(
                        ResourceSnippetParameters.builder()
                                .tag("유저픽 게시글")
                                .summary("해시태그 상품 추천 - 게시글 좋아요 api")
                                .description("해시태그 상품 추천 - 게시글 좋아요 api 입니다.")
                                .responseFields(responseFields)
                                .build()));
    }
    ...
    public void test1() throws Exception {
        ...
        // when
        ResultActions resultActions = getResultActions(post.getId(), session);
